...
public class LatestReleaseService : ILatestReleaseService
{
    ...
    public async Task<LatestRelease> Get(Guid id) => await _latestRelease.Find(a => a.Id == id).FirstOrDefaultAsync();
    ...
    public async Task Update(Guid id, LatestRelease version) =>
    ...
    public async Task Remove(Guid id) => await _latestRelease.DeleteOneAsync(a => a.Id == id);
